...
if ! docker run --platform linux/amd64 -e SCONE_NO_TIME_THREAD=1 --rm hello-world &> /dev/null
then
...
if ! ((kubectl get las | grep HEALTHY) && (kubectl get cas -A | grep HEALTHY) && (kubectl get sgxplugin | grep HEALTHY))
then
